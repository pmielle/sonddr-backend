...
import { deleteDocument, getDocument, postDocument, updateDocument } from "../database";

export async function onCommentVotePut(req: Request, res: Response) {
    // build
    ...
    // check the value
    if (! [-1, 1].includes(payload.value)) {
        res.status(400).send("Value must be -1 or 1");
        return;
    }
    // post
    await updateDocument(`comment-votes/${id}`, payload);
    // respond
    res.status(200).send();
    return;
}
...
export async function onCommentVotePost(req: Request, res: Response) {
    ...
    const value = getFromReqBody("value", req, true);
    const id = `${commentId}-${fromId}`;
    const commentVote: DbCommentVote = {_id: id, commentId: commentId, fromId: fromId, value: value};
    // check the value
